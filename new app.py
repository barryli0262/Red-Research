...
with st.sidebar:
    ...
    st.write("---")
    st.write("🔍 **核心功能**")
    st.caption("1. 避雷信息聚合：深度抓取全网负面反馈\n2. 问题归因分类：智能识别是品质还是售后\n3. 痛痒程度分级：区分微瑕疵与致命伤\n4. 角色视图翻译：为不同身份提供行动建议")
    st.write("---")
...
with col_left:
    ...
    fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 10])), showlegend=False, margin=dict(t=30, b=30, l=30, r=30), height=380)
