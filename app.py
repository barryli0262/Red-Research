...
st.markdown("""
    ...
    .note-card { background-color: #ffffff; padding: 15px; border-radius: 12px; border: 1px solid #eee; margin-bottom: 15px; height: 120px; transition: 0.3s; }
    ...
    .subtitle { color: #666666; font-size: 1.1em; margin-bottom: 20px; }
...
with st.sidebar:
    ...
    st.image("https://upload.wikimedia.org/wikipedia/commons/b/b4/Xiaohongshu_logo.png", width=80)
    st.write("---")
    st.write("🔍 **核心功能**")
    st.caption("1. 避雷信息聚合\n2. 问题归因分类\n3. 痛痒程度分级\n4. 角色视图翻译")
    st.write("---")
...
# 搜索与跳转逻辑
search_query = st.text_input("", placeholder="输入品牌或产品名，查看雷点严重程度...")
...
if search_query:
    for b_name in mock_db.keys():
        if b_name.lower() in search_query.lower():
            target_brand = b_name
            break
# 如果没搜到或没搜，给一个默认提示或通过侧边栏控制
if not target_brand:
    target_brand = "戴森" # 默认初始展示
...
with st.expander("📝 查看《避雷观察薯》产品设计白皮书 (PRD 核心)"):
    st.markdown("""
    ### 1. 产品核心价值
    **避雷观察薯** 解决的不是“信息不足”，而是“决策瘫痪”。它将小红书海量的原始避雷帖，通过 AI 转化为**结构化、可分级、可决策**的判断结果。
